...
var doGetModuleInput = function(variantId, moduleId, kpiId, cb) {
                ...
                if(err) {
                    ...
                    output.save(function(err) {
                        if(err) {
                            ...
                            console.log(msg);
                            cb(null, {userId: variant.userId ,message: msg});
                        }
                    });
                }
            });
...
var variant = {
    ...
    getModuleInputFramework: function(moduleData, userRepository, processRepository, cb) {
        doGetModuleInput(moduleData.variantId, moduleData.moduleId, moduleData.kpiId, function(err, moduleInput) {
            if(err) {
                cb(err);
            } else {
    ...
    getModuleInput: function(req, res) {
        ...
        if (req.isAuthenticated()) {
            ...
            doGetModuleInput(variantId, moduleId, kpiId, function(err, moduleInput) {
                if(err) {
                    res.status(500).json(err);
                } else {
    ...
    getModuleOutput: function(req, res) {
        ...
        if (req.isAuthenticated()) {
            ...
            if(variantId && moduleId && kpiId) {
                ...
                //         res.status(404).json({message: "Variant not found"});
                //     } else {
                //         var moduleOutput = _.find(variant.outputData, function(output) {return output.moduleId === moduleId && output.kpiId === output.kpiId;});
