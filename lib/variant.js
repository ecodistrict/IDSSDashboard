...
function _updateVariant(variantData, cb) {
    ...
    Variant.findOne({_id: variantData._id}, function(err, variant) {
        if(err) {
            ...
            if(variantData.selectedContext) {
                variant.selectedContext = variantData.selectedContext;
            }
...
function getVariants(req, res) {
    ...
    if(req.user.activeCaseId) {
        _getVariants(userId, req.user.activeCaseId.toString(), function(err, variants) {
            if(err) {
                return res.status(500).json(err);
            }
...
function _getVariants(userId, caseId, cb) {
    ...
    Variant.find({userId: userId, caseId: caseId}, 'name description kpiValues kpiDisabled selectedContext userId',function(err, variants) {
