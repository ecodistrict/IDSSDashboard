...
var variant = {
        ...
        if(moduleData.variantId && moduleData.moduleId && moduleData.kpiAlias) {
            Variant.findById(moduleData.variantId).lean().exec(function(err, variant) {
                ...
                if(err) {
                    ...
                    var found = _.find(variant.outputData, function(o) {return o.kpiAlias === moduleData.kpiAlias && o.moduleId === moduleData.moduleId;});
                    if(found) {
                        // replace if result data exists
                        console.log('output data was updated on module');
                        found.outputs = moduleOutput.outputs;
                        found.status = moduleOutput.status;

                        Variant.findOneAndUpdate({ _id: variant._id.toString() }, _.omit(variant, [ '_id', '__v' ]), function(err) {
                            if(err) {
                                cb(err);
                            } else {
    ...
    // remove ouput when kpi is unselected or when module is switched out
