    ...
    addInputSpecification: function(moduleData, cb) {
        ...
        Process.findById(moduleData.processId, function(err, process) {
            ...
            if(err) {
                ...
                kpi = _.find(process.kpiList, function(k) {return k.alias === moduleData.kpiId;});
                if(kpi) {
                    console.log('now add inputSpecification to kpiList in process');
                    kpi.selectedModule.inputSpecification = moduleData.inputSpecification;
                ...
                if(err) {
                    ...
                    Variant.find({processId: process._id}, function(err, variants) {
                        if(err) {
                            ...
                            Input.find({'variantId': {$in: variantIds}}).remove(function(err, inputs) {
                                if(err) {
                                    res.status(500).json(err);
                                } else {
