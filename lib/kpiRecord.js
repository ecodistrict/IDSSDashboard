                ...
                if (err) {
                    ...
                    Variant.find({processId: process._id}, function(err, variants) {
                        ...
                        if (err) {
                            ...
                            KpiRecord.find(query, 'kpiAlias moduleId userId value variantId disabled', function(err, records) {
                                if(err) {
                                    res.status(500).json(err);
                                } else {
    ...
    saveKpiRecord: function(req, res) {
        ...
        if (req.isAuthenticated()) {
            ...
            if(kpiRecord.variantId && kpiRecord.kpiAlias) {
                ...
                console.log(req.user.facilitatorId.equals(kpiRecord.userId));

                if(!kpiRecord.userId) {
                    kpiRecord.userId = req.user._id;
                } else if(req.user.facilitatorId.equals(kpiRecord.userId)) {
