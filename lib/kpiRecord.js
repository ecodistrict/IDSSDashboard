    ...
    if(userId && variantId && moduleId && kpiAlias) {
        KpiRecord.findOne({userId: userId, variantId: variantId, moduleId: moduleId, kpiAlias: kpiAlias}, function(err, existingRecord) {
            if(err) {
                ...
                console.log('record existed: ', existingRecord);    
    ...
    saveKpiRecord: function(req, res) {
        ...
        if (req.isAuthenticated()) {
            ...
            if(kpiRecord.variantId && kpiRecord.kpiAlias) {
                ...
                } else if(req.user.role === 'Stakeholder' && req.user.facilitatorId.equals(kpiRecord.userId)) {
                    // this kpi record was from facilitator (default)
                    // delete id so that kpi record can be inserted as copy on stakeholder
                    delete kpiRecord._id;
                    // add stakeholder as user
                    kpiRecord.userId = req.user._id;
                }
    ...
    deleteKpiRecordsByKpiAlias: function(req, res) {
        ...
        if (req.isAuthenticated()) {
            if(!kpiAlias) {
                ...
                processId = req.user.activeProcessId || 'dummy id';
