    ...
    if(userId && variantId && moduleId && kpiAlias) {
        KpiRecord.findOne({userId: userId, variantId: variantId, moduleId: moduleId, kpiAlias: kpiAlias}, function(err, existingRecord) {
            if(err) {
                ...
                console.log('record existed: ' + existingRecord.kpiAlias);    
...
var doSaveKpiRecord = function(kpiRecord, cb) {
    ...
    KpiRecord.findOne({variantId: kpiRecord.variantId, kpiAlias: kpiRecord.kpiAlias, userId: kpiRecord.userId}, function(err, existingRecord) {
        ...
        if(err) {
            ...
            // in case this record was from facilitator delete old id
            delete kpiRecord._id;
            ...
            if(kpiRecord.weight || kpiRecord.weight === 0) {
                existingRecord.weight = kpiRecord.weight;
            }

            if(kpiRecord.minimum || kpiRecord.minimum === 0) {
                existingRecord.minimum = kpiRecord.minimum;
            }
            ...
            if(kpiRecord.inputs && kpiRecord.moduleId) {
                existingRecord.moduleId = kpiRecord.moduleId;
...
var kpiRecord = {
    ...
    getKpiRecord: function (req, res) {
        ...
        if (req.isAuthenticated()) {
            ...
            // if stakeholder - get also facilitator records, 
            // return only one set with order 1. stakeholder 2. facilitator
            // this is because as-is and alternative records is from facilitator
            // and to-be could be overwritten by stakeholder, but still are defaulted to facilitator
            // thus stakeholder can not query by userid
            if(req.user.role === 'Stakeholder') {
                query.userId = {$in: [req.user._id, req.user.facilitatorId]};
            ...
            KpiRecord.find(query, function (err, records) {
                ...
                if (err) {
                    ...
                    // if stakeholder, filter on 1. stakeholder 2. facilitator
                    if(req.user.role === 'Stakeholder') {
                        recordExtract = _.filter(records, function(r) {return r.userId.equals(req.user._id);});
                        if(recordExtract.length === 0) {
                            recordExtract = _.filter(records, function(r) {return r.userId.equals(req.user.facilitatorId);});
                        }
                        res.json(200, recordExtract);
                    } else {
    ...
    getKpiRecords: function(req, res) {
        ...
        if(req.isAuthenticated()) {
            ...
            Case.findById(req.user.activeCaseId, function(err, process) {
                if (err) {
                    ...
                    Variant.find({processId: process._id}, function(err, variants) {
                        ...
                        if (err) {
                            ...
                            KpiRecord.find(query, 'kpiAlias moduleId userId value variantId weight minimum disabled', function(err, records) {
                                if(err) {
                                    res.status(500).json(err);
                                } else {
                                    var userIds = _.pluck(records, 'userId');
                                    User.find({'_id': {$in: userIds}}, 'fname lname name email facilitatorId', function(err, users) {
                                        if (err) {
                                            res.json(500, err);
                                        } else {
                                            res.json(200, {
    ...
    deleteKpiRecordsByKpiAlias: function(req, res) {
        ...
        if (req.isAuthenticated()) {
            if(!kpiAlias) {
                res.status(400).json({message: "KPI alias is missing"});
            } else {
                processId = req.user.activeCaseId || 'dummy id';
                Case.findOne({_id: processId, userId: req.user._id}, function(err, process) {
                    if(err) {
                        res.status(500).json(err);
                    } else if(!process) {
                        res.status(404).json({message: "Case not found or not authorized"});
                    } else {
                        Variant.find({processId: process._id}, function(err, variants) {
                            if(err) {
                                res.status(500).json(err);
                                ...
                                var variantIds = _.pluck(variants, '_id');
                                KpiRecord.find({'kpiAlias': kpiAlias, 'variantId': {$in: variantIds}}).remove(function(err, records) {
                                    if(err) {
                                        res.status(500).json(err);
                                    } else {
                                        Output.find({'kpiAlias': kpiAlias, 'variantId': {$in: variantIds}}).remove(function(err, inputs) {
                                            if(err) {
                                                res.status(500).json(err);
                                            } else {
                                                res.status(200).json({message: 'KPI records and outputs was deleted'});
                                            }
                                        });
                                    }
                                });
