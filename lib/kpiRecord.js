...
var doGetModuleInput = function(userId, variantId, moduleId, kpiAlias, asIsVariantId, cb) {
    ...
    if(userId && variantId && moduleId && kpiAlias) {
        KpiRecord.findOne({userId: userId, variantId: variantId, moduleId: moduleId, alias: kpiAlias}, function(err, existingRecord) {
            if(err) {
                cb(err);
            } else if(!existingRecord) {
                // init from as is if first time requested
                if(asIsVariantId) {
                    KpiRecord.findOne({userId: userId, variantId: asIsVariantId, moduleId: moduleId, alias: kpiAlias}, function(err, asIsRecord) {
                        var newRecord;
                        if(err) {
                            cb(err);
                        } else if(!asIsRecord) {
...
var kpiRecord = {
    ...
    getModuleInputFramework: function(moduleData, userRepository, processRepository, cb) {
        doGetModuleInput(moduleData.userId, moduleData.variantId, moduleData.moduleId, moduleData.kpiId, moduleData.asIsVariantId, function(err, moduleInput) {
            if(err) {
                cb(err);
            } else {
    ...
    saveKpiRecordFromModuleResult: function(moduleOutput, cb) {
        ...
        if(kpiOutput && (kpiOutput.value || kpiOutput.value === 0)) {
            ...
            moduleOutput.alias = moduleOutput.kpiId;
    ...
    saveKpiRecordStatus: function(moduleData, cb) {
        if(moduleData.kpiId) {
            moduleData.alias = moduleData.kpiId;
        }
        if(!moduleData.alias) {
            console.log('ERROR: kpi alias is not set');
        }
